...
```
{company}-{role}/
├── application.md          # FIXED FACTS: company, role, location, posting_url, saved_date, + verbatim JD
├── job.md                  # same + tier summary (from ranked pipeline)
├── verdict.json            # tier, optimization summary, plan path (from npm run apply)
├── review-feedback-v1.json # reviewer structured edits + narrative suggestions (from draft-review)
...
├── cv-v1.pdf  (+ .html)    # rendered by doc-export for the version you finalize
└── cover-letter-v1.pdf (+ .html)
```

**Flat, not nested** — no stage subfolders. **Versioning:** files start at `v1`; bump to `v2`/`v3` only for a distinct draft worth keeping (the versioned files *are* the history).

**`review-feedback-v1.json`** — written by `skills/draft-review/` after the reviewer subagent runs. Contains Part A (structured string-replacement edits) and Part B (narrative suggestions: missed keywords, company angles, reframing, tone). See `skills/draft-review/resources/review-schema.md`. Bump to `v2` only when re-running reviewer on a new draft version.

`doc-export` renders HTML+PDF only for the version you finalize.
