...
**Do NOT run during scrape/rank or for every discovered job.**

Only run when:
- The user **explicitly selects** a job and requests research, OR
- The job is **Tier S** AND the user has initiated the apply workflow (`npm run apply` completed)

If neither condition is met, skip company research and tell the user it runs after they commit to applying.
