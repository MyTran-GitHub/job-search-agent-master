...
3. **Fill the `<body>` from the markdown**, keeping the `<style>` block byte-for-byte unchanged (link CSS is already in the template). Follow the
   ...
   - **Links:** wrap every email and URL in `<a href="...">...</a>` — emails use `mailto:`, web URLs use `https://`.
     Bare domains like `linkedin.com/in/handle` or `github.com/user/repo` need `https://` in the href.
     Links render blue and underlined via template CSS (`color: #0563C1; text-decoration: underline`).
...
5. **Linkify** any bare URLs you missed (contact lines, project bullets):
   ```bash
   ...
   ```
6. **Auto-render the PDF** into the same folder using a browser the user already has (the kit ships none of its own):
...
7. When the user submits, set the tracker's `Shipped` to this version (`application-tracker`) — it maps to `cv-v{n}.pdf`.
